...
3. Structure Cleaning:
   ...
   - Optionally keeps ions (metal atoms and ionic compounds)
...
## Configurable OpenBabel Parameters:
   ...
   - Keep Ions: Retain ions and metal atoms in the structure (default False)

## Output:

   - Cleaned PDB file (protein-only structure, optionally with ions)
...
class ProteinWithIonsSelect(Select):
    """Select standard amino acid residues and ions"""
    def accept_residue(self, residue):
        # Accept standard amino acid residues (protein residues)
        if residue.id[0] == ' ':
            return True
        # Accept hetatoms that are ions (typically single atoms or simple molecules)
        # Common ion residues: NA (sodium), K (potassium), CA (calcium), MG (magnesium), 
        # ZN (zinc), FE (iron), CL (chloride), etc.
        if residue.id[0] == 'H':
            res_name = residue.resname.strip()
            # List of common ions to keep
            common_ions = {'NA', 'K', 'CA', 'MG', 'ZN', 'FE', 'CL', 'BR', 'I', 'F', 
                          'SO4', 'PO4', 'NO3', 'OH', 'O', 'HOH', 'WAT'}
            return res_name in common_ions
        return False
...
class ProteinNameWithIonsSelect(Select):
    """Select standard amino acid residues and ions from specific chains"""
    def __init__(self, chain_ids):
        """
        Parameters:
            chain_ids (list): List of chain identifiers to keep (e.g., ['A', 'B'])
...
def clean_and_convert_pdb_to_pdbqt(
    ...
    Parameters:
        ...
        - keep_ions (bool): Whether to keep ions and metal atoms in the structure (default False)
    ...
    # Select appropriate selector based on keep_ions parameter
    if keep_ions:
        if protein_name:
            selector = ProteinNameWithIonsSelect(target_chains)
        else:
            selector = ProteinWithIonsSelect()
        print(f"Keeping ions in the structure")
    else:
        selector = ProteinNameSelect(target_chains)
        print(f"Removing ions from the structure")
